            ...
            while (running) {
                ...
                switch (choice) {
                    ...
                    case 4:
                        ...
                        Movie filmToUpdate = null;

                        for (Movie movie : movieList) {
                            if (movie.getJudul().equalsIgnoreCase(judulToUpdate)) {
                                filmToUpdate = movie;
                                break;
                            }
                        }
                        ...
                        if (filmToUpdate != null) {
                            // Minta pengguna untuk memasukkan informasi baru
                            System.out.print("Masukkan Judul Baru (tekan Enter untuk tidak mengubah): ");
                            String newTitle = scanner.nextLine();
                            if (!newTitle.isEmpty()) {
                                filmToUpdate.setJudul(newTitle);
                            }

                            System.out.print("Masukkan Tahun Rilis Baru (tekan Enter untuk tidak mengubah): ");
                            String newYearInput = scanner.nextLine();
                            if (!newYearInput.isEmpty()) {
                                int newYear = Integer.parseInt(newYearInput);
                                filmToUpdate.setTahunRilis(newYear);
                            }

                            System.out.print("Masukkan Sutradara Baru (tekan Enter untuk tidak mengubah): ");
                            String newDirector = scanner.nextLine();
                            if (!newDirector.isEmpty()) {
                                filmToUpdate.setSutradara(newDirector);
                            }
